...
import EAPJsonData from '@db/EAP/EAP.json';
import { 
  Box, 
  Typography, 
  Paper, 
...
const createRows = (data: { uniprotid: string; genename: string; uniprotname: string }[]) => {
...
const pathwayGroups = Object.entries(EAPJsonData).map(([key, value]) => {
    return {
        pathwayName: key,
        data: createRows(value as { uniprotid: string; genename: string; uniprotname: string }[])
    };
});
        ...
        <Container maxWidth="lg">
            ...
            <Paper elevation={0} sx={{ p: 2, mb: 4, bgcolor: 'background.paper', borderRadius: 2 }}>
                <Typography variant="body1" color="text.secondary">
                    Below are protein pathways associated with epilepsy. Click on a gene name to search or Uniprot ID to view details.
                </Typography>
            </Paper>
            ...
            {isMobile ? (
                ...
                <Box sx={{ mb: 4 }}>
                    {pathwayGroups && pathwayGroups.map((group, index) => (
                        <Accordion 
                            key={index}
                ...
                <Box sx={{ mb: 4 }}>
                    {pathwayGroups && pathwayGroups.map((group, index) => (
                        <Card key={index} sx={{ mb: 4, borderRadius: 2, overflow: 'hidden' }}>
                            <CardHeader 
