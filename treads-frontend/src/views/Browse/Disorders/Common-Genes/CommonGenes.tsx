...
const ColumnGrouping = lazy(() => import('@components/Tables/MUI/ColumnGrouping'));
...
const FAVORITE_DISORDERS = ["MTLE", "MTLE-HS", "FCD", "HS", "DS", "CAE", "JAE", "JME", "EGTCS"];
  ...
  const fetchCommonGenes = async () => {
    ...
    try {
      ...
      const formattedRows = data.map((row: CommonGenesDataRaw) => {
        const references = row.references.map((ref: string) => `PMID: ${ref}`).join("; ")
      ...
      <Container maxWidth="lg">
        ...
        >
          <Typography variant="body1" color="text.secondary">
            Below are common genes across different disorders. Use the filter to find out the common genes across different disorders.
            Click on a gene name to search or Uniprot ID to view details.
          </Typography>
        </Paper>
                      ...
                      <FormControlLabel
                        ...
                        label={
                          <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                            {disorder}
                            <Link to={`/${disorder}`} style={{ display: 'flex', alignItems: 'center' }}>
