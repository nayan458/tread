...
const Navbar: React.FC = () => {
  const urls: URLItem[] = [
    ...
    { dropDown: true, label: 'Browse', urls: [
        ...
        { dropDown:true, label: 'Disorder', urls: [
    ...
    { to: '/EpilepsyAssociatedPathways', label: 'Epilepsy Associated Pathways' },
  ];
  ...
  return (
    <nav className="flex flex-col bg-background-secondary p-4 pt-12 space-y-2">
      {urls.map((url, index) => {
        ...
        return (
          <LeftNavLink key={index} to={url.to} label={url.label} />
        );
        }
        return <>Hello</>;
      })}
